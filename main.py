...
if len(os.listdir(f'./{TO_SORT_FOLDER}') ) != 0: 
    ...
    for path in Path(f'./{TO_SORT_FOLDER}').iterdir():
        ...
        if path.suffix in valid_filetypes:
            ...
            dest = Path(f'./{dest_folder}/{path.name}')
            
            # Don't need to create folders if missing, will be manually created
...
desk_width, desk_height = user32.GetSystemMetrics(78), user32.GetSystemMetrics(79)
...
for size in wallpaper_sizes:
    wallpaper_paths.append(f'./{size_folders[size]}/{choice(os.listdir(f"./{size_folders[size]}"))}')
...
for i, img in enumerate(wallpaper_images):
    ...
    if (width == desired_width) and (height == MONITOR_HEIGHT):
        ...
        if width / (SINGLE_ASPECT_RATIO * wallpaper_size) <= height:
            # Use width as datum
            img = crop_wallpaper(img, round(width / (SINGLE_ASPECT_RATIO * wallpaper_size)), width)
        else:
            # If height too small to use width as datum, use height instead
            img = crop_wallpaper(img, height, height * (SINGLE_ASPECT_RATIO * wallpaper_size))
...
if len(wallpaper_images) != 1:
    ...
    final_wallpaper = Image.new('RGB', (num_monitors * MONITOR_WIDTH, MONITOR_HEIGHT))
